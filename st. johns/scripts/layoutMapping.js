...
const EXTRA_FEATURES_SELECTOR = "#ctlBodyPane_ctl16_ctl01_gvwExtraFeatures";
  ...
  buildings.forEach((b, bIdx) => {
    ...
    if (bIdx < buildingAllLayouts.length) {
      ...
      buildingLevelLayouts.forEach((layout) => {
        ...
        if (layoutType[0]) {
          spaceTypeCounter.increment(layoutType[0]);
          const spaceTypeIndex = spaceTypeCounter.get(layoutType[0]);
          layouts.push(defaultLayout(layoutType[0], (bIdx + 1), `${(bIdx + 1)}.${(spaceTypeIndex)}`, null, toInt(layout.area), layoutType[1]));
        }
      });
    }
  });
...
function main() {
  ...
  const buildingLayouts = buildLayoutsFromBuildings(buildings, buildingAllLayouts);
