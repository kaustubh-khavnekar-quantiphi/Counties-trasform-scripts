  ...
  try {
    if (unnormalizedAddress && unnormalizedAddress.full_address) {
      ...
      const strTxt = $('td:contains("S/T/R")')
        ...
        .text()
        .trim(); // "26-8S-22E"
      if (strTxt && /\d{2}-\d{1,2}[A-Z]-\d{1,2}[A-Z]/.test(strTxt)) { // Updated regex for "26-8S-22E"
        const parts2 = strTxt.split("-");
        section = parts2[0];
        township = parts2[1]; // e.g., "8S"
        range = parts2[2]; // e.g., "22E"
      }

      // Extract street_name without directional abbreviations for schema validation
      let street_name_without_directional = street_name;
      if (pre) {
        const preRegex = new RegExp(`^${pre}\\s+`, 'i');
        street_name_without_directional = street_name_without_directional.replace(preRegex, '').trim();
      }
   
      writeJson(path.join("data", "address.json"), {
        street_number: street_number || null,
        street_pre_directional_text: pre || null,
        street_name: street_name ? street_name.toUpperCase() : null, // Ensure uppercase for consistency
        ...
        county_name: "Bradford", // Hardcoded as per the HTML source
        country_code: "US",
        latitude:
